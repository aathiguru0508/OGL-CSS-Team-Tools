document.getElementById('generateBtn').addEventListener('click', () => {
  ...
  appIds.forEach(appId => {
    apiNames.forEach(apiName => {
      const guideUrl = `https://guidedlearning.oracle.com/player/latest/api/scenario/simulation/see_it/${appId}/${apiName}/lang/--/?draft=dev&windowMode=unpin`;

      const tr = document.createElement('tr');
      tr.innerHTML = `
        <td>${appId}</td>
        <td>${apiName}</td>
        <td><a href="${guideUrl}" target="_blank">Video Link</a></td>
      `;
